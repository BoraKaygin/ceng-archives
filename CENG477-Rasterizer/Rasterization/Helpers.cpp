...
Vec4 vec3IntoVec4Point(const Vec3& v){
    return {v.x, v.y, v.z, 1.0};
}

Vec3 vec4IntoVec3(const Vec4& v) {
    return {v.x, v.y, v.z};
}

Vec4 perspectiveDivide(const Vec4& v) {
    return {v.x/v.t, v.y/v.t, v.z/v.t, 1.0};
}

Vec3 computeNormal(const Vec3& v1, const Vec3& v2, const Vec3& v3){
    Vec3 a = subtractVec3(v2, v1);
    Vec3 b = subtractVec3(v3, v1);
    return normalizeVec3(crossProductVec3(a, b));
}

void fitToImage(Vec3i& v, int width, int height){
    if (v.x < 0) {
        v.x = 0;
    } else if (v.x >= width) {
        v.x = width - 1;
